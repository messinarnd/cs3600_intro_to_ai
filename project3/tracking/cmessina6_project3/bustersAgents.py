...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        next_sq = None
        min_dist = float("inf")
        while count < len(livingGhostPositionDistributions):
            dist = self.distancer.getDistance(livingGhostPositionDistributions[count].argMax(), pacmanPosition)
            if min_dist >= dist:
                next_sq = livingGhostPositionDistributions[count].argMax()
                min_dist = dist
            count += 1

        count = 0
        min_dist = float("inf")
        while count < len(legal):
            dist = self.distancer.getDistance(next_sq, Actions.getSuccessor(pacmanPosition, legal[count]))
            if min_dist >= dist:
                optimal_act = legal[count]
                min_dist = dist
            count += 1
