...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is not None:
            ...
            while count < len(self.legalPositions):
                data = emissionModel[util.manhattanDistance(self.legalPositions[count], pacmanPosition)]
                if data > 0:
                    data2 = self.beliefs[self.legalPositions[count]] * data
                    possible[self.legalPositions[count]] = data2
                count += 1
        else:
            possible = util.Counter()
            possible[self.getJailPosition()] = 1
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        possible = util.Counter()
        while count < len(self.legalPositions):
            for updated_pos, probability in self.getPositionDistribution(self.setGhostPosition(gameState, self.legalPositions[count])).items():
                data = probability * self.beliefs[self.legalPositions[count]]
                possible[updated_pos] += data
            count += 1
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        count = 0
        self.particles = []
        while self.numParticles > count:
            second_count = 0
            while second_count < len(self.legalPositions):
                count += 1
                self.particles.append(self.legalPositions[second_count])
                second_count += 1
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is not None:
            ...
            while count < len(self.particles):
                possible[self.particles[count]] += emissionModel[util.manhattanDistance(self.particles[count], pacmanPosition)]
                count += 1
            if possible.totalCount() != 0:
                part_list = []
                possible.normalize()
                second_count = 0
                while second_count < self.numParticles:
    ...
    def elapseTime(self, gameState):
        ...
        part_list = []
        while count < len(self.particles):
            position = self.particles[count]
            count += 1
            data = self.setGhostPosition(gameState, position)
            pos_dist = self.getPositionDistribution(data)
            data2 = util.sample(pos_dist)
            part_list.append(data2)
        self.particles = part_list
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        while count < self.numParticles:
            second_count = 0
            while second_count < len(pos):
                self.particles.append(pos[second_count])
                second_count += 1
                count += 1
        ...
        while count < len(self.particles):
            ...
            while second_count < self.numGhosts:
                if noisyDistances[second_count] is not None:
                    w *= emissionModels[second_count][util.manhattanDistance(self.particles[count][second_count], pacmanPosition)]
                else:
                    self.particles[count] = self.getParticleWithGhostInJail(self.particles[count], second_count)
                second_count += 1
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            while count < self.numGhosts:
                data = setGhostPositions(gameState, newParticle)
                pos_dist = getPositionDistributionForGhost(data, count, self.ghostAgents[count])
                newParticle[count] = util.sample(pos_dist)
                count += 1
            "*** END YOUR CODE HERE ***"
